# Release Procedure for action-prepare-check-outputs
...
gh release create "${NEXT_VERSION}" \
  ...
  --generate-notes \
  --repo Framework-R-D/action-prepare-check-outputs
```
...
```bash
gh release view "${NEXT_VERSION}" --repo Framework-R-D/action-prepare-check-outputs
```
...
```bash
NEW_SHA=$(gh api repos/Framework-R-D/action-prepare-check-outputs/git/ref/tags/${NEXT_VERSION} \
  --jq .object.sha)
# If the tag is annotated, dereference:
TYPE=$(gh api repos/Framework-R-D/action-prepare-check-outputs/git/ref/tags/${NEXT_VERSION} \
  --jq .object.type)
if [ "$TYPE" = "tag" ]; then
  NEW_SHA=$(gh api repos/Framework-R-D/action-prepare-check-outputs/git/tags/${NEW_SHA} \
    --jq .object.sha)
fi

# Update README.md
OLD_PATTERN="Framework-R-D/action-prepare-check-outputs@[0-9a-f]\{40\} # v[0-9]*"
NEW_STRING="Framework-R-D/action-prepare-check-outputs@${NEW_SHA} # ${NEXT_VERSION}"
